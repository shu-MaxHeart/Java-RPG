...
public class plse {
  public static void main(String[] args) {
      ...
      if(args.length > 0) p[0] = new SWordfighter("ラン",Integer.parseInt(args[0]));
			//戦士を定義
      p[1] = new WIzard("ミキ",1);
      if(args.length > 1) p[1] = new WIzard("ミキ",Integer.parseInt(args[1]));
			//魔術師を定義
      p[2] = new MOnk("スゥ",1);
      if(args.length > 2) p[2] = new MOnk("スゥ",Integer.parseInt(args[2]));
			//勇者を定義
			p[3] = new Hero("ダイヤ",1);
      ...
      p[(int)(Math.random()*4)].attack(p[(int)(Math.random()*4)]);//回復
			*/
			for(int i = 1;;i++){
				...
				if((route > 3)&&(p[index].checklive())&&((p[index].job.equals("僧侶")||(p[index].job.equals("魔術師"))))){
					//index = 2;
					do{
						index2 = (int)(Math.random()*4);
					}while((index == index2)||!(p[index2].checklive()));
					p[index].attack_freim(p[index2]);//魔法攻撃
				}else if((route > 2)&&(p[index].checklive())&&((p[index].job.equals("僧侶")||(p[index].job.equals("魔術師"))))){
					//index = 2;
					/**do{
						index2 = (int)(Math.random()*4);
					}while((index == index2)||!(p[index2].checklive()));/**/
...
public class Hero {
  ...
  String job = "勇者";
	...
	void State_stream(String job){
		...
		str = FR.Reader_File("job_csv.csv",this.job);
		/*for(String x: str)
	...
	void attack(Hero e) {
		System.out.println(this.name + " の "+" 攻撃！");
			...
			damege = (int)(this.atk / 2 - e.dfe / 4 + rnd);
			critical_damege = (int)(this.atk / 0.6 + rnd * 1.5);
		if(!(this.checklive())){
			this.maxline(2);
		}else if(!(e.checklive())){
			e.maxline(2);
  ...
  void heal(Hero h) {
	...
	void attack_freim(Hero e) {
		...
		if(!(this.checklive())){
			...
			damege = (int)(this.mgk / 2 - e.mdf / 2 + rnd +25);
			...
			if(hit_rnd > hit_brns){
				...
				if(critical_damege<1){
  	 	  	...
  	 	  	e.hp -= critical_damege;
					if(0 > e.hp) e.hp = 0;
					e.maxline(2);
					if(Math.random()*100<60) get_exp(Math.round(( 4 + 3 * (e.Lv+0) * 3 / (this.Lv+0)) + second_attack(e,2)));
				...
				if(damege<1){
  	 	  	...
  	 	  	e.hp -= damege;
					if(0 > e.hp) e.hp = 0;
					e.maxline(2);
					if(Math.random()*100<60) get_exp(Math.round(( 4 + 3 * (e.Lv+0) / (this.Lv+0)) + second_attack(e,2)));
			...
			if(damege<1){
				...
				if(critical_damege<1){
  	 	  	...
  	 	  	e.hp -= critical_damege;
					if(0 > e.hp) e.hp = 0;
					e.maxline(2);
					if(Math.random()*100<(60/cnt)) plse_get = (Math.round(( 4 + 2 * (e.Lv+0) * 3 / (this.Lv+0)) + second_attack(e,++cnt)));
				...
				if(damege<1){
  	 	  	...
  	 	  	e.hp -= damege;
					if(0 > e.hp) e.hp = 0;
					e.maxline(2);
					if(Math.random()*100<(60/cnt))  plse_get = ((int)( 4 + 2 * (e.Lv+0) / (this.Lv+0)) + second_attack(e,++cnt));
